...
export default function InvoiceImportDialog() {
  ...
  const statusBadge = (status: string) => {
    ...
    return (
      <span className={`inline-block rounded-full px-2 py-0.5 text-[10px] font-semibold ${styles[status] || 'bg-secondary text-muted-foreground'}`}>
        {status}
      </span>
    )
  }
  ...
  return (
    <>
      <button
        ...
        className="inline-flex items-center gap-1.5 rounded-md bg-card px-3 py-1.5 text-sm font-medium text-foreground ring-1 ring-border ring-inset hover:bg-secondary transition-colors"
      >
      ...
      <Dialog open={open} onClose={() => setOpen(false)} className="relative z-50">
        ...
        <div className="fixed inset-0 flex items-center justify-center p-4">
          <DialogPanel className="w-full max-w-3xl rounded-xl bg-card shadow-2xl max-h-[85vh] flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto px-6 py-4">
              {phase === 'idle' && (
                <div className="text-center py-12">
                  <ArrowUpTrayIcon className="mx-auto size-12 text-muted-foreground/50" />
                  ...
                  <button
                    ...
                    className="mt-6 rounded-md bg-primary px-4 py-2 text-sm font-medium text-white hover:bg-primary/100 transition-colors"
                  >
                    Choose Files
                  </button>
                </div>
              ...
              {(phase === 'uploading' || phase === 'importing') && (
                <div className="text-center py-12">
                  <div className="mx-auto size-8 animate-spin rounded-full border-2 border-border border-t-indigo-600" />
                  <p className="mt-4 text-sm text-muted-foreground">
              ...
              {response && (phase === 'preview' || phase === 'done') && (
                <>
                  <div className="mb-4 flex items-center gap-4 text-sm">
                    <span className="font-medium text-foreground">{response.summary.total} invoices</span>
                    ...
                    <table className="min-w-full text-sm">
                      <thead>
                        <tr className="border-b border-border">
                          ...
                          <th className="py-2 px-3 text-left text-xs font-medium text-muted-foreground">Plate</th>
                          ...
                          <th className="py-2 pl-3 text-left text-xs font-medium text-muted-foreground">Status</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-border">
                        {response.results.map((r, i) => (
                          <tr key={i} className="hover:bg-secondary">
                            <td className="py-1.5 pr-3 font-medium text-foreground">{r.invoiceNumber ?? '—'}</td>
                            <td className="py-1.5 px-3 text-muted-foreground">{r.date ?? '—'}</td>
                            <td className="py-1.5 px-3 text-muted-foreground max-w-[200px] truncate">{r.customer ?? '—'}</td>
                            <td className="py-1.5 px-3 font-mono text-xs text-muted-foreground">{r.plate ?? '—'}</td>
                            <td className="py-1.5 px-3 text-right text-foreground">{r.total?.toFixed(2) ?? '—'}</td>
                            <td className="py-1.5 px-3 text-right text-muted-foreground">{r.lineCount}</td>
              ...
              {phase === 'preview' && (
                <>
                  ...
                  <button
                    ...
                    className="rounded-md bg-primary px-4 py-2 text-sm font-medium text-white hover:bg-primary/100 transition-colors"
                  >
                    Confirm Import
                  </button>
                </>
