    ...
    const normalButtons = options.filter(x => !x.inMenu);
        ...
        {normalButtons.map((opt,index) => {
            ...
            const className = clsx(
                 ...
                 index==(normalButtons.length-1)&&menuButtons.length==0&&"rounded-r-md", 
                    opt.isPrimary ? "relative inline-flex items-center  -ml-px bg-primary px-3 py-2 text-sm font-semibold text-white   hover:bg-primary/100 focus:z-10" 
                                    :clsx(opt.redText?"text-red-900 ring-red-300 hover:bg-red-50":"text-foreground ring-border hover:bg-secondary", "relative  inline-flex items-end  -ml-px bg-card px-3 py-2 text-sm font-semibold  ring-1 ring-inset  focus:z-10"))
        ...
        {menuButtons.length>0 && 
          <Menu as="div" className="relative -ml-px block">
            <MenuButton className="relative  inline-flex items-end rounded-r-md bg-card px-2 py-2 text-muted-foreground ring-1 ring-border ring-inset hover:bg-secondary focus:z-10">
            ...
            <MenuItems
                transition
                modal={false}
                className=" absolute bottom-full  right-0 z-20 mb-2 -mr-1 w-56 origin-top-right rounded-md bg-card ring-1 shadow-lg ring-black/5 transition focus:outline-hidden data-closed:scale-95 data-closed:transform data-closed:opacity-0 data-enter:duration-100 data-enter:ease-out data-leave:duration-75 data-leave:ease-in">
                <div className="py-1 z-20">
                    {options.filter(x => x.inMenu).map((opt,index) => {
                        const className = clsx(opt.redText?"text-red-700   data-focus:text-red-900":"text-foreground   data-focus:text-foreground", "block z-20 px-4 py-2 text-sm  data-focus:outline-hidden") 
                        return (
                            <MenuItem key={index}>
                               {opt.href ?
