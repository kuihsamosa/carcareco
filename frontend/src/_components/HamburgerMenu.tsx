...
export default function HamburgerMenu({
    ...
    options: IButtonOption[]
     ...
     const menuItemClassName = "block px-3 py-1 text-sm/6 text-foreground data-focus:bg-secondary data-focus:outline-hidden";
     const redMenuItemClassName = "block px-3 py-1 text-sm/6 text-red-900 data-focus:bg-red-50 data-focus:outline-hidden";
    const  className="absolute right-0 z-10 mt-2 w-32 origin-top-right rounded-md bg-card py-2 ring-1 shadow-lg ring-gray-900/5 transition focus:outline-hidden data-closed:scale-95 data-closed:transform data-closed:opacity-0 data-enter:duration-100 data-enter:ease-out data-leave:duration-75 data-leave:ease-in";
    if(options.length==0) return <></>
    return (
         <Menu as="div" className="relative flex-none">
            <MenuButton className="-m-2.5 block pb-2.5 text-muted-foreground hover:text-foreground">
